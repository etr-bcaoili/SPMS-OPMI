...
'FOR MDI
 ...
 End If
  ...
  cnn.Execute "Delete From Rawdata Where CompanyCode = '" & "MDI" & "' AND CUTMO = '" & txtMonth.Text & "' AND CUTYEAR = '" & cmbYear.Text & "'"
...
'    cmdDeleteBillingOrder.Execute
    ...
    cnn.Execute "Insert Into UploadedRawData(CompanyCode,CheckFileName,Cutmo,Cutyear,TotalGrossAmount,TotalQuantity,TotalNetAmount,TotalProductDisc) " & _
        " Values('" & "MDI" & "','" & Right(txtfile, 12) & "','" & txtMonth.Text & "','" & cmbYear.Text & "'," & TotalGrossAmount & "," & TotalQuantity & "," & TotalNetAmount & "," & TotalProductDisc & " )"
...
For i = 1 To ln
   ...
   ' If co = "MD" And prn = "650" Then
    ...
    Set cmdRawData = New ADODB.Command
        ...
        cmdRawData.Parameters.Append cmdRawData.CreateParameter("@CompanyCode", adVarChar, adParamInput, 10, "MDI")
