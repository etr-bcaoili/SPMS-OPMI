...
Begin VB.Form FrmSalesTerritorialRegionUplaoding 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Sales Territorial Region Uploading"
   ClientHeight    =   3450
   ClientLeft      =   4185
   ClientTop       =   3990
   ClientWidth     =   7095
   ...
   Begin VB.Label Label5 
      BackStyle       =   0  'Transparent
      Caption         =   "Sales Territorial Region"
      BeginProperty Font 
         Name            =   "Segoe UI"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmSalesTerritorialRegionUplaoding"
...
i = 2
   ...
   ' If co = "MD" And prn = "650" Then
        ...
        rsSalesMatrix.Open "Select STTERRCD,STITMDIVCD from SalesMatrix Where STDISTRCTCD = '" & Replace(Trim(objWorksheet.Cells(i, 2)), "'", "") & "'", cnn, adOpenKeyset
        ...
        If rsItem.State = 1 Then rsItem.Close
            rsItem.Open "Select * from Item Where Itemcd = '" & Replace(Trim(objWorksheet.Cells(i, 3)), "'", "") & "' and Itemdivcd = '" & rsSalesMatrix.Fields(1).Value & "'", cnn, adOpenKeyset
        ...
        cmdSalesTargetUploading.Parameters.Append cmdSalesTargetUploading.CreateParameter("@STTERRCD", adVarChar, adParamInput, 25, rsSalesMatrix.Fields(0).Value)
        cmdSalesTargetUploading.Parameters.Append cmdSalesTargetUploading.CreateParameter("@YEAR", adInteger, adParamInput, , Replace(Trim(objWorksheet.Cells(i, 4)), "'", ""))
        cmdSalesTargetUploading.Parameters.Append cmdSalesTargetUploading.CreateParameter("@MONTH", adVarChar, adParamInput, 2, Replace(Trim(objWorksheet.Cells(i, 5)), "'", ""))
        ...
        cmdSalesTargetUploading.Parameters.Append cmdSalesTargetUploading.CreateParameter("@Quantity", adNumeric, adParamInput, , Trim(objWorksheet.Cells(i, 6)))
        cmdSalesTargetUploading.Parameters("@Quantity").Precision = 11
        cmdSalesTargetUploading.Parameters("@Quantity").NumericScale = 6
        cmdSalesTargetUploading.Parameters.Append cmdSalesTargetUploading.CreateParameter("@Amount", adDouble, adParamInput, , Trim(objWorksheet.Cells(i, 8)))
        cmdSalesTargetUploading.Parameters.Append cmdSalesTargetUploading.CreateParameter("@SalesTarget", adDouble, adParamInput, , Trim(objWorksheet.Cells(i, 9)))
