...
If rsSalesTarget.State = 1 Then rsSalesTarget.Close
    ...
    If cmbDistrict.Text <> "" And cmbDistrict.Text <> "All" Then
        query = query & " AND D.STDISTRCTCD = '" & cmbDistrict.Text & "' "
    End If
    
    If cmbItemDivision.Text <> "" And cmbItemDivision.Text <> "All" Then
...
ProgressBar1.Value = 0
    ...
    For x = 2 To rsSalesTarget.RecordCount + 1
         ...
         Sheet1.Cells(x, 1) = rsSalesTarget.Fields("COMID").Value
         Sheet1.Cells(x, 2) = rsSalesTarget.Fields("STTERRCD").Value
         Sheet1.Cells(x, 3) = "'" & rsSalesTarget.Fields("ITEMCD").Value
         If rsItem.State = 1 Then rsItem.Close
            rsItem.Open "Select * from Item where Itemcd = '" & rsSalesTarget.Fields("ITEMCD").Value & "' And Itemdel = '" & "0" & "'", cnn, adOpenKeyset
         Sheet1.Cells(x, 4) = rsItem.Fields("IMDBRN").Value
         Sheet1.Cells(x, 5) = rsSalesTarget.Fields("YEAR").Value
         Sheet1.Cells(x, 6) = "'" & rsSalesTarget.Fields("MONTH").Value
         If rsCalendar.State = 1 Then rsCalendar.Close
            rsCalendar.Open "Select * from Calendar where CAYR = '" & rsSalesTarget.Fields("YEAR").Value & "' AND  CAPN = '" & rsSalesTarget.Fields("MONTH").Value & "' AND IsActive = '" & "1" & "'", cnn, adOpenKeyset
            
         Sheet1.Cells(x, 7) = rsCalendar.Fields("MonthDescription").Value
         Sheet1.Cells(x, 8) = rsSalesTarget.Fields("Quantity").Value
         Sheet1.Cells(x, 9) = rsItem.Fields("IMDFRM").Value
         Sheet1.Cells(x, 10) = rsSalesTarget.Fields("Amount").Value
         Sheet1.Cells(x, 11) = rsSalesTarget.Fields("SalesTarget").Value
...
If rsDistrict.State = 1 Then rsDistrict.Close
rsDistrict.Open "select Distinct stdistrctcd  from stdistrict where dltflg = 0", cnn, adOpenKeyset
cmbDistrict.Clear
cmbDistrict.AddItem "All"
For x = 1 To rsDistrict.RecordCount
cmbDistrict.AddItem rsDistrict.Fields(0).Value
rsDistrict.MoveNext
Next x
