...
Public Class ProductGroupSharingUploading
	...
	Public Property MonthTo As String
		...
		End Set
	End Property
	Private Shared Function BaseFilter(ByVal Table As System.Data.DataTable) As ProductGroupSharingUploadingCollection
		Dim col As New ProductGroupSharingUploadingCollection
		For j As Integer = 0 To Table.Rows.Count - 1
			Dim m_ProductGroupSharingUploading As New ProductGroupSharingUploading
			Dim row As DataRow = Table.Rows(j)
			m_ProductGroupSharingUploading.m_Year = row("Year")
			m_ProductGroupSharingUploading.m_Month = row("Month")
			m_ProductGroupSharingUploading.m_ConfigtypeCode = row("ConfigtypeCode")
			m_ProductGroupSharingUploading.m_ChannelCode = row("ChannelCode")
			m_ProductGroupSharingUploading.m_CustomerCode = row("CustomerCode")
			m_ProductGroupSharingUploading.m_CustomerName = row("CustomerName")
			m_ProductGroupSharingUploading.m_ShiptoCode = row("ShiptoCode")
			m_ProductGroupSharingUploading.m_ProductGroupCode = row("ProductGroup")
			m_ProductGroupSharingUploading.m_SalesAgentCode = row("SalesAgentCode")
			m_ProductGroupSharingUploading.m_Shared = row("Shared")
			m_ProductGroupSharingUploading.m_CRTBY = row("CRTBY")
			m_ProductGroupSharingUploading.m_IsActive = row("IsActive")
			m_ProductGroupSharingUploading.m_OR_SH = row("OR-SH")
			col.Add(m_ProductGroupSharingUploading)
		Next
		Return col
	End Function
	Public Function Save() As Boolean
		...
		Try
			Dim cmd As New SqlCommand("uspProductGroupSharing", SPMSOPCI.ConnectionModule.SPMSConn2)
			...
			cmd.Parameters.AddWithValue("@ProductGroupCode", m_ProductGroupCode)
	...
	Public Shared Function CheckUploadingFileExist(ByVal Month As String, ByVal Year As String, ByVal ConfigCode As String, ByVal ChannelCode As String) As Boolean
		Return ExecuteCommand("SELECT 'A' FROM [ProductGroupSharing] Where Month = '" & Month & "' And Year = '" & Year & "' And ConfigtypeCode = '" & ConfigCode & "' And ChannelCode = '" & ChannelCode & "'") = "A"
	End Function
	Public Shared Function DeleteExistList(ByVal Month As String, ByVal Year As String, ByVal ConfigCode As String, ByVal ChannelCode As String) As Boolean
		Return ExecuteCommand("DELETE FROM [ProductGroupSharing] Where ChannelCode = '" & ChannelCode & "' AND Year = '" & Year & "' AND Month = '" & Month & "' AND ConfigtypeCode = '" & ConfigCode & "'")
	End Function
	Public Shared Function GetItemSharingListSql() As String
		Return "Select Distinct ConfigtypeCode,ChannelCode,Year,Month,ConfigtypeCode From [ProductGroupSharing] Order by Year,Month,ChannelCode"
	End Function
	Public Shared Function GetCustomerItemSharingListSql(ByVal ChannelCode As String, ByVal Year As String, ByVal Month As String, ByVal ConfigtypeCode As String) As String
		Return "Select Distinct ChannelCode,Year,Month,ConfigtypeCode From [ProductGroupSharing] Where ChannelCode = '" & ChannelCode & "' And Year = '" & Year & "' And Month = '" & Month & "' And ConfigtypeCode = '" & ConfigtypeCode & "'"
	End Function
End Class
Public Class ProductGroupSharingUploadingCollection
	Inherits List(Of ProductGroupSharingUploading)
End Class