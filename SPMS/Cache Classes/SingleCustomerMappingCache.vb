...
Public Class SingleCustomerMappingCache
    ...
    Private Function LoadDataTable(ByVal COMID As String, ByVal FromDate As Date, ByVal ToDate As Date) As DataTable
        ...
        cmd.Parameters.AddWithValue("@Action", "SingleCustomermapping")
        cmd.Parameters.AddWithValue("@COMID", COMID)
        cmd.Parameters.AddWithValue("@EffectivityStartDate", FromDate.ToShortDateString)
        cmd.Parameters.AddWithValue("@EffectivityEndDate", ToDate.ToShortDateString)
    ...
    Public Function FilterDv() As DataView
        ...
        If m_Channel <> "" Then
            m_Filter = "COMID like '%" & HandleSingleQuoteInSql(RefineFilter(m_Channel)) & "%' "
        End If
        If m_CustomerCode <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "CustomerCD like '%" & HandleSingleQuoteInSql(RefineFilter(m_CustomerCode)) & "%' "
        End If
        If m_ShipToCode <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "CDACD like '%" & HandleSingleQuoteInSql(RefineFilter(m_ShipToCode)) & "%' "
        End If
        If m_ShipToName <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "CDAName like '%" & HandleSingleQuoteInSql(RefineFilter(m_ShipToName)) & "%' "
        End If
        If m_RegionCode <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "RegCd like '%" & HandleSingleQuoteInSql(RefineFilter(m_RegionCode)) & "%' "
        End If
        If m_Province <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "DistrctCd like '%" & HandleSingleQuoteInSql(RefineFilter(m_Province)) & "%' "
        End If
        If m_Municipality <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "AreaCd like '%" & HandleSingleQuoteInSql(RefineFilter(m_Municipality)) & "%' "
        End If
        If m_Territory <> "" Then
            If m_Filter <> "" Then m_Filter &= " AND "
            m_Filter &= "AreaCovrg like '%" & HandleSingleQuoteInSql(RefineFilter(m_Territory)) & "%' "
        End If
