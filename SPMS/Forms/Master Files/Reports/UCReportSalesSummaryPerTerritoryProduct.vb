...
Public Class UCReportSalesSummaryPerTerritoryProduct
    ...
    Private Sub btnPrintPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintPreview.Click
        ...
        GetData(" EXECUTE uspSPMSGPISalesSummaryTerritoryProduct @BeginMonth='" & cmbStartMonth.Text & "',  @BeginYear=" & cmbStartYear.Text & ", @EndMonth='" & cmbEndMonth.Text & "',  @EndYear=" & cmbEndYear.Text & ", @ProductCode = '" & cmbproduct.Text & "' ")
...
errHandler:
    ...
    Private Function Rsitems(Optional ByVal Filter As String = "") As ADODB.Recordset
        Dim rs As New ADODB.Recordset
        If Filter <> "" Then
            Filter = " AND " & Filter
        End If
        rs.Open("SELECT * FROM ITEM WHERE ITEMDEL = 0 " & Filter, SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
    ...
    Private Sub PopulateComboBox()
        ...
        cmbproductdesc.Items.Clear()

        'for the items
        rs = Rsitems()
        If rs.RecordCount > 0 Then
            For i As Integer = 1 To rs.RecordCount
                cmbproduct.Items.Add(rs.Fields("Itemcd1").Value)
                cmbproductdesc.Items.Add(rs.Fields("imdbrn").Value)
                rs.MoveNext()
            Next
        End If
    ...
    Private Sub cmbproduct_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbproduct.SelectedIndexChanged
        Dim rs As New ADODB.Recordset
        rs = Rsitems("ITEMCD1 = '" & HandleSingleQuoteInSql(cmbproduct.Text) & "' ")
        If rs.RecordCount > 0 Then
            cmbproductdesc.Text = rs.Fields("imdbrn").Value
        End If
    End Sub

    Private Sub cmbproductdesc_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbproductdesc.SelectedIndexChanged
        Dim rs As New ADODB.Recordset
        rs = Rsitems("imdbrn = '" & HandleSingleQuoteInSql(cmbproductdesc.Text) & "' ")
        If rs.RecordCount > 0 Then
            cmbproduct.Text = rs.Fields("itemcd1").Value
        End If
    End Sub
