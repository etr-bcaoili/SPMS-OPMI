...
Public Class UCReportSalesSummaryProductRegion
    ...
    Private Sub btnPrintPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintPreview.Click
        ...
        GetData(" EXECUTE uspSPMSGPISalesSummaryProductRegion @BeginMonth=" & cmbStartMonth.Text & ",  @BeginYear=" & cmbStartYear.Text & ", @EndMonth=" & cmbEndMonth.Text & ",  @EndYear=" & cmbEndYear.Text & ", @RegionCode = '" & cmbRegionCode.Text & "'  ")
        'GetData(" EXECUTE uspSPMSGPISalesSummaryProductRegion @BeginMonth='" & cmbStartMonth.Text & "',  @BeginYear=" & cmbStartYear.Text & ", @EndMonth='" & cmbEndMonth.Text & "',  @EndYear=" & cmbEndYear.Text & ", @RegionCode = '" & cmbRegionCode.Text & "'  ")
        'Dim i
...
errHandler:
    ...
    Private Function RsGpiCalendarMonth(Optional ByVal Filter As String = "") As ADODB.Recordset
        ...
        If Filter <> "" Then
            Filter = " AND " & Filter
        End If
        rs.Open("SELECT DISTINCT CAPN FROM CALENDAR WHERE COMID = '" & RsCompany.Fields("ComID").Value & "' " & Filter, SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
    ...
    Private Function RsGpiCalendarYear(Optional ByVal Filter As String = "") As ADODB.Recordset
        ...
        If Filter <> "" Then
            Filter = " AND " & Filter
        End If
        rs.Open("SELECT DISTINCT CAYR FROM CALENDAR WHERE COMID = '" & RsCompany.Fields("ComID").Value & "' " & Filter, SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
    ...
    Private Function RsRegion(Optional ByVal Filter As String = "") As ADODB.Recordset
        ...
        End If
        rs.Open("SElECT * FROM STREGION WHERE DLTFLG = 0 " & Filter, SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
        Return rs
    End Function
    ...
    Private Sub PopulateComboBox()
        ...
        If rs.RecordCount > 0 Then
            For i As Integer = 1 To rs.RecordCount
                ...
                cmbregiondesc.Items.Add(rs.Fields("StRegname").Value)
                rs.MoveNext()
            Next
        End If
    ...
    Private Sub cmbRegionCode_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbRegionCode.SelectedIndexChanged
        Dim rs As New ADODB.Recordset
        rs = RsRegion(" STRegCD = '" & cmbRegionCode.Text & "' ")
        If rs.RecordCount > 0 Then
            cmbregiondesc.Text = rs.Fields("StRegName").Value
        End If
    End Sub
    ...
    Private Sub cmbregiondesc_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbregiondesc.SelectedIndexChanged
        Dim rs As New ADODB.Recordset
        rs = RsRegion("StRegName = '" & cmbregiondesc.Text & "' ")
        If rs.RecordCount > 0 Then
