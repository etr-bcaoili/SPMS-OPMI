...
Public Class ucSalesTransactionByTerritoryCode

    Private bindingSource1 As New BindingSource()
    Private dataAdapter As New SqlDataAdapter()
    Private table As New DataTable()
    Private c_TerritoryCode As String
    ...
    Private Function ValidateData() As Boolean
        ...
        If cmbTerritory.Text = String.Empty Then
            m_Err.SetError(cmbTerritory, "Territory is required")
            m_HasError = True
        End If
    ...
    Private Sub btnPrintPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintPreview.Click
        ...
        If ValidateData() Then
            If Not chkExclude.Checked Then
                GetData(" EXECUTE uspSPMSGPISalesTransactionListForThePeriodOFByTerritory @TerritoryCode='" & Trim(cmbTerritoryCode.Text) & "', @BeginMonth=" & cmbStartMonth.Text & ",  @BeginYear=" & cmbStartYear.Text & ", @EndMonth=" & cmbEndMonth.Text & ",  @EndYear= " & cmbEndYear.Text)
            Else
                GetData(" EXECUTE uspSPMSGPISalesTransactionListForThePeriodOfByTerritorywithExclusion @TerritoryCode='" & Trim(cmbTerritoryCode.Text) & "', @BeginMonth=" & cmbStartMonth.Text & ",  @BeginYear=" & cmbStartYear.Text & ", @EndMonth=" & cmbEndMonth.Text & ",  @EndYear= " & cmbEndYear.Text)
            End If
        End If
    End Sub
    ...
    Private Sub Export()
        ...
        objSheet.Name = "Sales Transaction By TerrCode"
        ...
        objBook.SaveAs("Sales Transaction By Territory Code", Excel.XlFileFormat.xlWorkbookNormal, System.Reflection.Missing.Value, System.Reflection.Missing.Value, True, False, Excel.XlSaveAsAccessMode.xlNoChange, False, False, System.Reflection.Missing.Value, System.Reflection.Missing.Value, System.Reflection.Missing.Value)
        Exit Sub
errHandler:
    ...
    Private Function RsGpiCalendarYear(Optional ByVal Filter As String = "") As ADODB.Recordset
        ...
        Return rs
    End Function
    Private Function TerritoryCode() As ADODB.Recordset
        Dim rs As New ADODB.Recordset
        Dim j

        rs.Open("SELECT DISTINCT STACOVCD,STACOVNAME FROM STAREACOVERAGE WHERE DLTFLG = 0 AND IsActive = 1", SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
        For j = 1 To rs.RecordCount
            If cmbTerritoryCode.Items.Contains(rs.Fields("STACOVCD").Value) = False Then
                cmbTerritoryCode.Items.Add(rs.Fields("STACOVCD").Value)
            End If
            If cmbTerritory.Items.Contains(rs.Fields("STACOVNAME").Value) = False Then
                cmbTerritory.Items.Add(rs.Fields("STACOVNAME").Value)
            End If
            rs.MoveNext()
        Next
        cmbTerritoryCode.Sorted = True
        cmbTerritory.Sorted = True
        Return rs
    End Function
    ...
    Private Sub cmbTerritoryName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbTerritory.SelectedIndexChanged
        Dim rs As New ADODB.Recordset

        'rs.Open("SELECT DISTINCT STTERRCD FROM STTERRITORY WHERE STTERRNAME = '" & cmbTerritory.Text & "' ", SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
        rs.Open("SELECT DISTINCT STACOVCD FROM STAREACOVERAGE  WHERE STACOVNAME = '" & cmbTerritory.Text & "' ", SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)

        'c_TerritoryCode = rs.Fields("STAcovcd").Value
        cmbTerritoryCode.Text = rs.Fields("STAcovCD").Value
    ...
    Private Sub cboTerritoryCode_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbTerritoryCode.SelectedIndexChanged
        Dim rs As New ADODB.Recordset

        'rs.Open("SELECT DISTINCT STTERRCD FROM STTERRITORY WHERE STTERRNAME = '" & cmbTerritory.Text & "' ", SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
        rs.Open("SELECT DISTINCT STACOVName FROM STAREACOVERAGE  WHERE STACOVCD = '" & cmbTerritoryCode.Text & "' ", SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)

        'c_TerritoryCode = rs.Fields("STAcovcd").Value
        cmbTerritory.Text = rs.Fields("STAcovName").Value
    End Sub
End Class
