...
Public Class UCRawDataAnalyzer
    ...
    Private Function CheckIfMonthAndYearIsLocked(ByVal Month As String, ByVal Year As String, ByVal CompanyCode As String, ByVal ConfigtypeCode As String) As Boolean
        Dim rs As New ADODB.Recordset
        If rs.State = 1 Then rs.Close()
        rs.Open("SELECT 'A' FROM Sc02 where  LockProcess = 1 AND  Month = '" & Month & "' AND year = '" & Year & "' AND [Company code] = '" & CompanyCode & "' AND Configtypecode = '" & ConfigtypeCode & "'", SPMSConn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockReadOnly)
        If rs.RecordCount > 0 Then
            Return True
        Else
            Return False
    ...
    Private Sub LoadTotalSaleproduct()
        ...
        If tb.Rows.Count > 0 Then
            For i As Integer = 0 To tb.Rows.Count - 1
                ...
                Dim row As DataGridViewRow = GviewTables.Rows(GviewTables.Rows.Add)
                ...
                row.Cells(colSaleAmount.Index).Value = dr("SALES AMOUNT")
            Next
        Else
            GviewTables.Rows.Clear()
        End If
    ...
    Private Sub AddSharingToTerritory(ByVal row As DataGridViewRow)
        Try
            SPMSConn.Execute("EXEC uspSharing @Action = 'SAVE',@Recid = '" & m_Recid & "',@Comid = '" & cboCompany.Text & "',@Cutmo = '" & cboMonth.Text & "',@Cutyear = '" & cboCalendarYear.Text & "', " & _
                             ...
                             "@delflag = 0,@Isactive = 1,@ConfigTypeCode = '" & cboConfig.Text & "'")
        Catch ex As Exception
    ...
    Private Sub lnkBatch_LinkClicked_1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkBatch.LinkClicked
    ...
    Private Sub cboCalendarYear_SelectedIndexChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboCalendarYear.SelectedIndexChanged
    ...
    Private Sub ToolStripButton1_Click(sender As Object, e As EventArgs) Handles ToolStripButton1.Click
        If cboConfig.Text <> "" Or cboCompany.Text <> "" Or cboCalendarYear.Text <> "" Or cboMonth.Text <> "" Then
            If ShowLockTransaction() = DialogResult.Yes Then
                LockTransaction(cboConfig.Text, cboCompany.Text, cboCalendarYear.Text, cboMonth.Text)
            End If
        End If
    End Sub
